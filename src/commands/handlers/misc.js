...
var handlers = {
    ...
    BATCH: function(command, handler) {
        ...
        handler.emit('batch start', cache);
        handler.emit('batch start ' + cache.type, cache);
        cache.commands.forEach(c => handler.executeCommand(c));
        handler.emit('batch end', cache);
        handler.emit('batch end ' + cache.type, cache);
    }
};
